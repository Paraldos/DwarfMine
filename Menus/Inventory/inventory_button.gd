...
func _ready():
	...
	InventoryController.switch_set.connect(_on_reset_button)
	_change_color()
...
func _physics_process(_delta):
	if Input.is_action_just_pressed("switch_items") && has_focus() && item:
		...
		InventoryController.inventory_description_update.emit(item, _exchange_is_valid())
...
func _exchange_is_valid():
	...
	if !has_focus():
		...
		return true
	elif selected_item && slot[0] == selected_item.type.to_lower():
		return true
	elif (slot[0] == "weapon_1" or slot[0] == "weapon_2") and selected_item.type == "Weapon":
		return true
	elif slot[0] == "bag" and !item:
		return true
	elif slot[0] == "bag" and InventoryController.selected_slot[0] == "bag":
		return true
	elif item && selected_item && item.type == selected_item.type:
		return true
	else:
		return false
...
func _change_color(color = null):
	if color:
		self_modulate = color
	elif InventoryController.selected_slot and !_exchange_is_valid():
